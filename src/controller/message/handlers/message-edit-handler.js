...
module.exports = class MessageEditHandler extends DefaultHandler {
  ...
  handle(message) {
    ...
    if (typeof message.payload.message.id !== 'string' || typeof message.payload.message.text !== 'string') {
    ...
    if (userConnection) {
      const messageFrom = {
        id: null,
        type: process.env.MSG_EDITED_FROM_SERVER,
        payload: result,
      };
